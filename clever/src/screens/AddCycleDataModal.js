...
const AddCycleDataModal = ({ visible, onClose, onSave, date, initialData }) => {
  ...
  const [symptoms, setSymptoms] = useState([]); // Store symptoms as an array
  ...
  const renderButton = (title, currentValue, setValue, multiple = false) => (
    <TouchableOpacity
      ...
      onPress={() => {
        if (multiple) {
          // Toggle selection for multiple symptoms (symptoms)
      ...
      <SafeAreaView style={styles.modalContainer}>
        ...
        <View style={styles.buttonContainer}>
          {["Cramps", "Fatigue", "Mood Swings", "Bloating"].map(
            (option) => renderButton(option, symptoms, setSymptoms, true) // Passing 'true' for multiple selections
          )}
        </View>
